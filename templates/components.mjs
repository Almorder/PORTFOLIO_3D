...
export function logoPreloader({hold=120}={}){
  return `<div class="brand-preloader" data-brand-preloader data-preloader-hold="${Number(hold)||120}" aria-hidden="true"><div class="brand-preloader__spotlight"></div><div class="brand-preloader__mark"><span>Nolan</span><b>.</b><span>Arc</span></div><i></i></div>`;
}
...
export function animatedStats(stats=[], {theme='dark', layout='auto', replay=false, animation='blur'}={}){
  if(!stats.length) return '';
  const styles=['blur','slide','fade','scale'];
  const selected=styles.includes(animation)?animation:'blur';
  return `<section class="animated-stats animated-stats--${esc(theme)} motion-reveal" data-animated-stats data-layout="${esc(layout)}" data-replay="${replay?'true':'false'}" data-animation="${esc(selected)}" aria-label="Repères concrets">${stats.map((stat,i)=>{
