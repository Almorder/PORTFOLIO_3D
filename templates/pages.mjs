...
export function homePage(){
  ...
  return head({
    ...
    path:'/', image:`${site.domain}/assets/og-home.png`, imageAlt:'Nolan Arc — halo elliptique noir, blanc et orange', imageWidth:'1024', imageHeight:'1024', bodyClass:'page-home page-fabrica page-v16'
  }) + header('/') + `
  <main id="main-content" class="v16-home">
    ...
    <section class="home-pricing v16-pricing motion-reveal" id="pricing" data-pricing-switcher>
      ...
      <div class="v16-pricing__tabs" role="tablist" aria-label="Choisir une offre"><button id="pricing-tab-wedding" type="button" role="tab" aria-selected="true" aria-controls="pricing-panel-wedding" data-pricing-tab="wedding">Mariage</button><button id="pricing-tab-direction" type="button" role="tab" aria-selected="false" aria-controls="pricing-panel-direction" data-pricing-tab="direction">Direction artistique</button><button id="pricing-tab-strategy" type="button" role="tab" aria-selected="false" aria-controls="pricing-panel-strategy" data-pricing-tab="strategy">Stratégie de marque</button></div>
      <div class="v16-pricing__panels">
        <article id="pricing-panel-wedding" role="tabpanel" aria-labelledby="pricing-tab-wedding" data-pricing-panel="wedding" class="is-active"><small>Film de mariage</small><strong>À partir de 1 500 €</strong><ul><li><b>30 min</b><span>film final</span></li><li><b>12 h → 00 h</b><span>couverture de la journée</span></li><li><b>Sur devis</b><span>options et besoins supplémentaires</span></li></ul><a href="/contact/?intent=moment">Parler du mariage ↗</a></article>
        <article id="pricing-panel-direction" role="tabpanel" aria-labelledby="pricing-tab-direction" data-pricing-panel="direction" hidden><small>Direction artistique</small><strong>Interventions ciblées dès 200 €</strong><p>Un point d’entrée pour un besoin précis. Le périmètre et les livrables évoluent ensuite selon ce qu’il faut créer, corriger ou harmoniser.</p><a href="/contact/?intent=brand">Présenter le projet ↗</a></article>
        <article id="pricing-panel-strategy" role="tabpanel" aria-labelledby="pricing-tab-strategy" data-pricing-panel="strategy" hidden><small>Conseil en stratégie de marque</small><strong>Session ciblée — 89 € / h</strong><p>Audit, priorisation ou session de travail ciblée.</p><p><b>Accompagnement — sur devis.</b></p><a href="/contact/?intent=brand">Réserver une session ↗</a></article>
      </div>
    </section>
    ...
    <section class="v16-testimonials" id="retours">
      ...
      <div class="v16-testimonial-bento">${site.testimonials.map((t,i)=>`<article class="motion-reveal ${i===0?'is-featured':''}"><div class="v16-stars" aria-label="Retour client positif">★★★★★</div><blockquote>« ${esc(t.quote)} »</blockquote><footer><strong>${esc(t.name)}</strong>${t.role?`<span>${esc(t.role)}</span>`:''}</footer></article>`).join('')}</div>
    </section>
...
export function workPage(){
  ...
  return head({title:'Work — Nolan Arc', description:'Films et projets sélectionnés de Nolan Ribeiro.', path:'/work/', bodyClass:'page-work page-fabrica page-v16'}) + header('/work/') + `<main id="main-content" class="fab-work-page v16-work-page">
    ...
    <section class="v16-expertise-switcher motion-reveal" data-expertise-switcher>
      ...
      <div class="v16-expertise-tabs" role="tablist" aria-label="Choisir une expertise">${expertiseTabs.map((x,i)=>`<button id="work-expertise-tab-${x.key}" type="button" role="tab" data-expertise-tab="${x.key}" aria-selected="${i===0?'true':'false'}" aria-controls="work-expertise-panel-${x.key}"><span>${x.index}</span>${x.label}</button>`).join('')}</div>
      <div class="v16-expertise-panels">${expertiseTabs.map((x,i)=>`<article id="work-expertise-panel-${x.key}" role="tabpanel" aria-labelledby="work-expertise-tab-${x.key}" data-expertise-panel="${x.key}"${i?' hidden':''}><small>${x.label}</small><h3>${x.headline}</h3><p>${x.text}</p><strong>${x.value}</strong><a href="${x.href}">Voir comment j’interviens ↗</a></article>`).join('')}</div>
    </section>
  </main>` + footer();
}
...
export function legalPage(){
  ...
  <h2>Éditeur du site</h2><p><strong>${esc(site.legalName)}</strong>${legal.businessForm ? ` — ${esc(legal.businessForm)}` : ''}, exerçant sous l’identité professionnelle <strong>${esc(site.name)}</strong>.<br>Email : <a href="mailto:${site.email}">${esc(site.email)}</a>.${legal.publicLegalContactsConfirmed && legal.professionalAddress ? `<br>Adresse professionnelle : ${esc(legal.professionalAddress)}.` : ''}${legal.publicLegalContactsConfirmed && legal.professionalPhone ? `<br>Téléphone professionnel : ${esc(legal.professionalPhone)}.` : ''}<br>SIREN : ${legalValue(legal.siren)}.<br>SIRET : ${legalValue(legal.siret)}.<br>Code APE : ${legalValue(legal.ape)}.<br>TVA : ${vat}.</p>
  <h2>Directeur de la publication</h2><p>${esc(legal.publicationDirector)}.</p>
  <h2>Hébergement</h2><p>${esc(legal.host.name)}, ${esc(legal.host.address)} — <a href="${esc(legal.host.website)}" rel="noreferrer">${esc(legal.host.website)}</a>.${legal.host.phone ? ` Téléphone : ${esc(legal.host.phone)}.` : legal.host.phoneNote ? ` ${esc(legal.host.phoneNote)}` : ''}</p>
...
export function privacyPage(){
  ...
  <h2>Destinataires et sous-traitant du formulaire</h2><p>Les données sont accessibles à Nolan Arc et transitent, pour l’envoi technique du formulaire, par ${esc(legal.formProcessor.name)}. Ce service agit comme intermédiaire technique pour transmettre le message ; ses propres conditions de traitement et de sécurité s’appliquent également. Aucune donnée du formulaire n’est utilisée par Nolan Arc à des fins publicitaires.</p>
  ...
  <h2>Contenus et services tiers</h2><p>Le fond vidéo YouTube de l’accueil reste désactivé tant que vous n’avez pas autorisé les contenus vidéo externes. Les autres lecteurs YouTube sont chargés uniquement après une action explicite dans leur emplacement. Les polices Yrsa, Syne et DM Sans sont chargées via Google Fonts afin de respecter l’identité graphique du site ; votre navigateur contacte donc les serveurs de ce prestataire lors du chargement des polices. Les liens vers Instagram, YouTube et Calendly ouvrent les services concernés ; leurs propres politiques s’appliquent lorsque vous quittez nolanarc.com.</p>
  ...
  <p class="legal-update">Dernière mise à jour : 14 août 2026 — V20 harmonisée.</p></article></main>` + footer();
}
...
export function cookiesPage(){
  ...
  <h2>Vidéo YouTube</h2><p>Aucun lecteur YouTube n’est chargé avant votre choix. Sur l’accueil, vous pouvez autoriser ou refuser le fond vidéo ; votre préférence est conservée dans un cookie fonctionnel pendant six mois et peut être modifiée depuis le footer. Sur les pages projet, l’activation du lecteur constitue un choix contextuel pour la lecture demandée. Après autorisation, votre navigateur communique avec YouTube et ce service peut utiliser des traceurs selon ses propres règles.</p>
  ...
  <h2>Votre choix</h2><p>Le refus est aussi simple que l’acceptation. Vous pouvez rouvrir le choix « Préférences vidéo » depuis le footer à tout moment. Si vous retirez votre autorisation, le lecteur externe du fond d’accueil est déchargé et n’est plus recréé lors des visites suivantes.</p><h2>Évolution du site</h2><p>Si un outil de mesure d’audience ou tout autre traceur non exempté devait être ajouté ultérieurement, le mécanisme de consentement devra être adapté avant son activation.</p>
  <p class="legal-update">Dernière mise à jour : 14 août 2026 — V20 harmonisée.</p></article></main>` + footer();
}

export function cgvPage(){
  const mediator = legal.mediator.name ? `${esc(legal.mediator.name)} — ${esc(legal.mediator.address)} — <a href="${esc(legal.mediator.website)}" rel="noreferrer">${esc(legal.mediator.website)}</a>` : `Aucun médiateur de la consommation n’est actuellement indiqué, l’adhésion devant être conclue par le professionnel avant que ses coordonnées puissent être publiées. Pour toute réclamation préalable : <a href="mailto:${site.email}">${esc(site.email)}</a>.`;
  ...
  <h2>4. Paiement</h2><p>${legalValue(legal.paymentTerms, 'Conditions de paiement à définir : échéancier, acompte éventuel et date d’exigibilité.')}</p><p><strong>Clients professionnels :</strong> pénalités de retard : ${legalValue(legal.latePenaltyB2B, 'Taux de pénalités de retard B2B à définir dans le respect du minimum légal.')}. Une indemnité forfaitaire de 40 € pour frais de recouvrement est due dans les conditions prévues par la loi. Escompte : ${legalValue(legal.discountTerms, 'Conditions d’escompte à définir, ou indiquer explicitement « aucun escompte ».')}.</p><p><strong>Clients consommateurs :</strong> Nolan Arc n’applique pas de doublement automatique du prix ni de pénalité punitive forfaitaire. En cas d’impayé, Nolan Arc peut suspendre la prestation et/ou la livraison et exercer les voies de recouvrement permises par le droit applicable.</p>
  <h2>5. Organisation, délais, livrables et collaboration</h2><p>Le calendrier est défini au devis et dépend notamment de la remise des éléments par le client, des validations, des disponibilités de tournage et des éventuels prestataires tiers. Les retards causés par l’absence de validation ou de fourniture des éléments nécessaires peuvent entraîner un décalage raisonnable du calendrier.</p><p>${esc(legal.archivalPolicy)}</p>
  ...
  <h2>7. Annulation, report ou fin anticipée</h2><p>${legalValue(legal.cancellationPolicy, 'Politique d’annulation/report et sort des sommes versées à définir selon votre pratique commerciale.')}</p><p><strong>Missions continues :</strong> ${esc(legal.continuousCancellationPolicy)}</p><p>Les droits impératifs du consommateur, notamment le droit de rétractation lorsqu’il s’applique, restent réservés.</p>
  ...
  <h2>11. Force majeure et responsabilité</h2><p>${esc(legal.forceMajeurePolicy)}</p><p>La responsabilité de Nolan Arc s’apprécie au regard de la mission effectivement convenue, sans priver un consommateur des droits que la loi lui reconnaît.</p>
